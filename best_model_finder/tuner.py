...
from Visualizations.visualization_graph import  visualization_graph
...
class Model_Finder:
    ...
    def get_best_params_for_KNN(self, train_x, train_y):
        ...
        try:
            ...
            # Creating an object of the Grid Search class
            self.grid = GridSearchCV(self.knn, self.param_grid_knn,
                                     cv=5, verbose=3)
            # finding the best parameters
            self.grid.fit(train_x, train_y)
            ...
            self.logger_object.log(self.file_object, 'KNN best params: ' + str(
    ...
    def get_best_params_for_SVM(self, train_x, train_y):
        ...
        """
        self.logger_object.log(self.file_object,
                               'Entered the get_best_params_for_Ensembled_Naive bayes method of the Model_Finder class')
        try:
            # initializing with different combination of parameters
            self.param_grid_svm = {'C': [0.1, 1, 10, 100, 1000],
                                   'kernel': ['rbf']}

            # Creating an object of the Grid Search class
            self.grid = GridSearchCV(self.svc, self.param_grid_svm,
                                     cv=5, verbose=3)
            # finding the best parameters
            self.grid.fit(train_x, train_y)
            ...
            # creating a new model with the best parameters
            self.svc = SVC(C=self.c, kernel=self.kernel, probability=True)
            # training the mew model
            self.svc.fit(train_x, train_y)
            self.logger_object.log(self.file_object, 'svc best params: ' + str(
                self.grid.best_params_) + '. Exited the svc method of the Model_Finder class')
            return self.svc
        except Exception as e:
            self.logger_object.log(self.file_object,
                                   'Exception occured in svc method of the Model_Finder class. Exception message:  ' + str(
                                       e))
            self.logger_object.log(self.file_object,
                                   'svc Parameter tuning  failed. Exited the svc method of the Model_Finder class')
            raise Exception()

    def get_best_model(self, train_x, train_y, test_x, test_y, cluster_number):
        """
        ...
        try:
            ...
            model_accuracies["KNN"] = self.knn_score
            ...
            model_accuracies["RandomForest"] = self.random_forest_score
            # create best model for SVM
            self.svc = self.get_best_params_for_SVM(train_x, train_y)
            self.prediction_svc = self.svc.predict_proba(
                test_x)  # prediction using the Random Forest Algorithm

            if len(test_y.unique()) == 1:  # if there is only one label in y, then roc_auc_score returns error. We will use accuracy in that case
                self.svc_score = accuracy_score((test_y), self.prediction_svc)
                self.logger_object.log(self.file_object, 'Accuracy for SVM:' + str(self.svc_score))
            else:
                self.svc_score = roc_auc_score((test_y), self.prediction_svc,
                                               multi_class='ovr')  # AUC for SVM
                self.logger_object.log(self.file_object, 'AUC for SVM:' + str(self.svc_score))
            model_accuracies["SVM"] = self.svc_score
            # comparing the two cluster_models
            print("Model Accuracies :: ", model_accuracies)
            visual.visualize_accuracy(model_accuracies, cluster_number)
            sorted_accuracy = sorted(model_accuracies.items(), key=lambda x: x[1], reverse=True)

            if (sorted_accuracy[0][0] == "KNN"):
                return 'KNN', self.knn
            elif (sorted_accuracy[0][0] == "RandomForest"):
                return 'RandomForest', self.random_forest
            elif (sorted_accuracy[0][0] == "SVM"):
                return 'SVM', self.svc
